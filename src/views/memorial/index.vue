  ...
  <div class="searchArea">
    <Form ref="formRef" layout="inline" @submit="state.reset">
      <Row style="width: 100%" :gutter="15">
        <Col :span="5">
          <Button type="primary" @click="router.push('/memorial/create-or-update')">新增大事记</Button>
        </Col>
        <Col :span="6">
          <Form.Item label="时间" name="time">
            <DatePicker.RangePicker v-model:value="time" @change="setTime" :placeholder="['开始日期', '结束日期']" />
          </Form.Item>
        </Col>
        <Col :span="7">
...
import { ref } from 'vue'
import { DatePicker, Button, Row, Col, Tooltip, Input, Form, Table, message } from 'ant-design-vue'
...
function setTime(e: any) {
  if (e) {
    state.searchParams.beginEventTime = dayjs(e[0]).format('YYYY-MM-DD HH:MM:ss')
    state.searchParams.endEventTime = dayjs(e[1]).format('YYYY-MM-DD HH:MM:ss')
  } else {
    state.searchParams.beginEventTime = undefined
    state.searchParams.endEventTime = undefined
  }
}

function handleEdit(id: any) {
  router.push({
    path: '/memorial/create-or-update',
    query: { id }
  })
}

function handleDetail(record: any) {
  let year = new Date(record.publishTime).getFullYear()
  window.open(global_config.EPORTAL_WEB + `#/memorial?id=${record.id}&year=${year}`, '_blank')
}
</script>
